...
/**
 * Utility used to load jcef in a maven generated environment
 *
 * @author Fritz Windisch
 */
public class JCefLoader {
    public static final String JAVA_LIBRARY_PATH = "java.library.path";
    public static final String JCEF_VERSION = "{native_version}-{os}{arch}";
    public static final String NATIVES_NAME = "jcef-natives-{os}{arch}.jar";
    ...
    public static synchronized CefApp installAndLoadCef(File installDir, CefSettings settings, String... args)
        ...
        if(new File(installDir, LOCK_FILE_NAME).exists()){
            ...
            versionDir = new File(installDir, JCEF_VERSION
                    ...
                    .replace("{arch}", OSUtil.getJvmArch()));
        }
        ...
        if(!versionDir.exists() || !lockFile.exists()){
            ...
            //Extract new version
            versionDir.mkdirs();
            String nativeResoureName = NATIVES_NAME
                    .replace("{os}", OSUtil.getOsName())
                    .replace("{arch}", OSUtil.getJvmArch());
            InputStream resource = JCefLoader.class.getResourceAsStream("/"+nativeResoureName);
            if(resource==null){
                try {
                    resource = new FileInputStream(nativeResoureName);
                }catch (Exception e) {
                    throw new RuntimeException("Could not initialize cef: " +
                            "Native resource " + nativeResoureName + " not found! Did you include it?", e);
                }
            }
